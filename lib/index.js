...
var SheetToArrayOfObjects = function (config, callback) {
	...
	http.get("http://spreadsheets.google.com/feeds/cells/" + SpreadSheetKey + "/1/public/basic?alt=json",
			function (response) {
				...
				response.on('end', function () {
					...
					try {
						...
						var parsed = convertArrayToArrayOfObjects(parsedBody);
						callback(null, parsed, parsedBody.feed.title.$t);
					} catch (ex) {
						callback(ex);
					}
				});
